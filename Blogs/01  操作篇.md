...
- 说明文档写的很干货，主要是没有例子，你不知道怎么使用。
- 对 JavaScript 语言的支持程度的提升空间还很大，目前知道的是已有的宏和 V8 引擎不兼容等。
- 只支持 Windows 系统，也就是说该软件对命令行的支持其实并不是主打功能。
...
不过，这个软件不支持 Linux 版本，因此想要在 Linux 系统上实现标签管理，这个内容我也需要去学习学习。
